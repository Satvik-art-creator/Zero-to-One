...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Welcome back, ${data.student.name.split(' ')[0]}! 🛡️`);
      navigate('/admin');
    } catch (err) {
      ...
      <div className="desktop-block" style={{
        ...
        <div style={{ zIndex: 1, marginTop: '-10vh' }}>
          ...
          <h1 style={{ fontSize: '3rem', lineHeight: 1.1, color: '#FFFFFF', marginBottom: '20px', fontFamily: 'var(--font-display)', letterSpacing: '-0.03em' }}>
            Placement<br/>Command Center.
          </h1>
          <p style={{ color: '#A09DBE', fontSize: '1rem', lineHeight: 1.6, maxWidth: '400px' }}>
            Manage companies, track applications, and oversee the entire placement season from one dashboard.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '380px' }}>
          <div style={{ marginBottom: '40px' }}>
            <h2 style={{ fontSize: '2rem', color: '#111827', marginBottom: '8px', letterSpacing: '-0.02em' }}>Admin Sign In</h2>
            <p style={{ color: '#6B7280', fontSize: '0.95rem' }}>Access the placement management panel.</p>
          </div>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '0.75rem', fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em' }}>TNP Email</label>
              <input
                type="email"
                name="email"
                required
                placeholder="name@tnp.iiitn.ac.in"
          ...
          <p style={{ textAlign: 'center', marginTop: '24px', fontSize: '0.9rem', color: '#6B7280' }}>
            Don't have an admin account? <Link to="/admin/register" style={{ color: '#5A52E5', textDecoration: 'none', fontWeight: 600 }}>Register here</Link>
          </p>
